...
use crate::{Query, QueryRef, Input};
...
impl System {
    ...
    /// This function requires `mut` to prevent input being set during query.
    /// TODO: In theory we could introduce a trait Queryable with only `query` and `query_ref`.
    pub async fn set_input<Q: Input + Query>(&mut self, query: Q, data: <Q as Query>::Output) {
